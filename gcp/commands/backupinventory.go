...
func (m *BackupInventoryModule) Execute(ctx context.Context, logger internal.Logger) {
	...
	if len(m.Resources) == 0 && len(m.Snapshots) == 0 {
	...
	logger.SuccessM(fmt.Sprintf("Found %d resource(s): %d protected, %d unprotected, %d snapshot(s)",
		len(m.Resources), protectedCount, unprotectedCount, len(m.Snapshots)), GCP_BACKUPINVENTORY_MODULE_NAME)
...
func (m *BackupInventoryModule) identifyUnprotectedResources() {
	...
	for diskURL, info := range m.allDisks {
		if !m.disksWithBackups[diskURL] {
			...
			// Add to loot
			m.LootMap["backup-inventory-commands"].Contents += fmt.Sprintf(
...
func (m *BackupInventoryModule) initializeLootFiles() {
	m.LootMap["backup-inventory-commands"] = &internal.LootFile{
...
func (m *BackupInventoryModule) writeOutput(ctx context.Context, logger internal.Logger) {
	...
	// Main backup inventory table (all resources)
	...
	// Snapshots table (one row per IAM binding member)
